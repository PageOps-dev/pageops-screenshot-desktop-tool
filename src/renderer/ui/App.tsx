...
export function App() {
  ...
  }, [apiKey, apiBaseUrl, targetUrl, mode, viewportW, viewportH, selector, headersJson, cookiesJson, watermark, posterConfig, presets, settingsLoaded]);
  ...
  async function addWatermarkToImage(imageUrl: string, config: WatermarkConfig): Promise<HTMLCanvasElement> {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Draw background and text with opacity
        const bgX = x - padding;
      ...
      <div className="panel">
        ...
        <div className="field">
          <div className="label" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <span
              onClick={() => window.pageops.openExternal('https://app.page-ops.com')}
              style={{ fontSize: 12, color: '#4a9eff', textDecoration: 'none', fontWeight: 'normal', cursor: 'pointer' }}
            >
              🔑 Get Free API Key →
            </span>
          </div>
        ...
        {showPosterPanel && (
          <>
            <div className="field">
              ...
              {(posterConfig.templateId === 'instagram-story') && (
                <div style={{ marginTop: 6, padding: '6px 10px', background: 'rgba(99,102,241,0.12)', borderRadius: 6, fontSize: 12, color: '#a5b4fc', lineHeight: 1.5 }}>
                  💡 Story (9:16) works best with a mobile viewport (e.g. 390×844) or a full-page screenshot.
                </div>
              )}
              {(posterConfig.templateId === 'instagram-product') && (
                <div style={{ marginTop: 6, padding: '6px 10px', background: 'rgba(99,102,241,0.12)', borderRadius: 6, fontSize: 12, color: '#a5b4fc', lineHeight: 1.5 }}>
                  💡 Product (4:5) works best with a tablet viewport (e.g. 768×1024) or a full-page screenshot.
                </div>
              )}
            </div>
